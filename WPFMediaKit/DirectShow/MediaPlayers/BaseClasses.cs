...
namespace WPFMediaKit.DirectShow.MediaPlayers
{
    ...
    public abstract class MediaPlayerBase : WorkDispatcherObject
    {
        ...
        protected virtual void SetupFilterGraph(IFilterGraph graph)
        {
			...
			// Custom
			if (m_SyncSourceSet)
				SyncSource = m_SyncSource;	// Update the filter after graph is set.
			// End Custom.
        }
		...
		bool m_SyncSourceSet;
		IReferenceClock m_SyncSource;
		public IReferenceClock SyncSource
		{
			get { return m_SyncSource; }
			set
			{
				m_SyncSourceSet = true;
				m_SyncSource = value;
				IMediaFilter mfilter;
				if (m_graph != null && (mfilter = m_graph as IMediaFilter) != null)
				{
					mfilter.SetSyncSource( m_SyncSource );
				}
			}
		}
