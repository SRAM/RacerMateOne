...
//! constructor
CBackDropSceneNode::CBackDropSceneNode(video::ITexture* top, video::ITexture* bottom, video::ITexture* left,
			video::ITexture* right, video::ITexture* front, video::ITexture* back, ISceneNode* parent, ISceneManager* mgr, s32 id)
: ISceneNode(parent, mgr, id)
{
	#ifdef _DEBUG
	setDebugName("CBackDropSceneNode");
	#endif
...
//! renders the node.
void CBackDropSceneNode::render()
{
...
const core::aabbox3d<f32>& CBackDropSceneNode::getBoundingBox() const
{
	return Box;
}


void CBackDropSceneNode::OnRegisterSceneNode()
{
	if (IsVisible)
...
video::SMaterial& CBackDropSceneNode::getMaterial(u32 i)
{
	return Material[i];
}
...
u32 CBackDropSceneNode::getMaterialCount() const
{
	return 6;
}
...
ISceneNode* CBackDropSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
	if (!newParent) newParent = Parent;
	if (!newManager) newManager = SceneManager;

	CBackDropSceneNode* nb = new CBackDropSceneNode(0,0,0,0,0,0, newParent,
		newManager, ID);
