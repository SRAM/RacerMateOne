...
CSkywallTiledSceneNode::CSkywallTiledSceneNode(video::ITexture* sky, video::ITexture* floor, u32 horiRes, u32 vertRes,
...
{
	#ifdef _DEBUG
	setDebugName("CSkywallTiledSceneNode");
	#endif
...
CSkywallTiledSceneNode::~CSkywallTiledSceneNode()
{
	if (Buffer)
		Buffer->drop();
}
...
void CSkywallTiledSceneNode::generateMesh()
{
	f32 azimuth;
	u32 k;
	...
	for (k = 0, azimuth = 0; k <= HorizontalResolution; ++k)
	{
		...
		{
			f32 cosEr = Radius;// * cosf(elevation);
			if(j==0)
				cosEr = Radius * cosf(elevation);
...
//! renders the node.
void CSkywallTiledSceneNode::render()
{
...
const core::aabbox3d<f32>& CSkywallTiledSceneNode::getBoundingBox() const
{
	return Buffer->BoundingBox;
}


void CSkywallTiledSceneNode::OnRegisterSceneNode()
{
	if (IsVisible)
	{
...
video::SMaterial& CSkywallTiledSceneNode::getMaterial(u32 i)
{
...
u32 CSkywallTiledSceneNode::getMaterialCount() const
{
	return 1;
}
...
void CSkywallTiledSceneNode::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const
{
...
void CSkywallTiledSceneNode::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options)
{
...
ISceneNode* CSkywallTiledSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
	if (!newParent) 
		newParent = Parent;
	if (!newManager) 
	...
	CSkywallTiledSceneNode* nb = new CSkywallTiledSceneNode(Buffer->Material.TextureLayer[0].Texture, Material.TextureLayer[0].Texture, HorizontalResolution, VerticalResolution, TexturePercentage, 
...
void CSkywallTiledSceneNode::setUVRect(const core::rect<f32>& uvRect)
{
	UVRect = uvRect;
...
void CSkywallTiledSceneNode::getUVRect(core::rect<f32>& uvRect) const
{
	uvRect = UVRect;
}
