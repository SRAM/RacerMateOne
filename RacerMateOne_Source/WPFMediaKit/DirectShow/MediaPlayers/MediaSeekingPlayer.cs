    ...
    public abstract class MediaSeekingPlayer : MediaPlayerBase
    {
        ...
        public double SpeedRatio
        {
            ...
            set
            {
                ...
                LastHR = m_mediaSeeking.SetRate(rate);
            }
        }
		public int LastHR { get; protected set; }
        ...
        protected void SetMediaSeekingInterface(IMediaSeeking mediaSeeking)
        {
			...
			AverageTimePerFrame = GetAverageTimePerFrame();
		...
		#region Custom
		public double AverageTimePerFrame { get; protected set; }
		
		static Guid CLSID_AviSplitter = new Guid(0x1b544c20, 0xfd0b, 0x11ce, 0x8c, 0x63, 0x0, 0xaa, 0x00, 0x44, 0xb5, 0x1e);
		static Guid FORMAT_VideoInfo = new Guid(0x05589f80, 0xc356, 0x11ce, 0xbf, 0x01, 0x00, 0xaa, 0x00, 0x55, 0x59, 0x5a);
		double GetAverageTimePerFrame()
		{
			double avgtime = 0.0f;
			IGraphBuilder graph = m_mediaSeeking as IGraphBuilder;
			if (graph != null)
			{
				IEnumFilters enumFilters;
				if (graph.EnumFilters(out enumFilters) == 0)
				{
					enumFilters.Reset();
					...
					{
						filters[0].QueryFilterInfo(out finfo);
						filters[0].GetClassID(out clsid);
						if (CLSID_AviSplitter == clsid)
						{
							IEnumPins enumpins;
							filters[0].EnumPins(out enumpins);
							var ipens = new IPin[1];
							while (enumpins.Next(1, ipens, fetched) == 0)
							{
								PinInfo pinfo;
								ipens[0].QueryPinInfo(out pinfo);
								if (pinfo.dir == PinDirection.Output)
								{
									AMMediaType pmt = new AMMediaType();
									ipens[0].ConnectionMediaType(pmt);
									if (pmt.formatType == FORMAT_VideoInfo)
									{
										VideoInfoHeader vih = new VideoInfoHeader();
										if (pmt.formatPtr != IntPtr.Zero)
										{
											Marshal.PtrToStructure(pmt.formatPtr, vih);
											avgtime = (double)vih.AvgTimePerFrame / 10000000.0; // In seconds!
										}
									}
								}
							}
