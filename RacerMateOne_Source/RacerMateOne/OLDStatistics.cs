...
namespace RacerMateOne
{
	...
	public class Statistics: RM1.IStatsEx
	{
		...
		public class Filter
		{
			...
			public Filter() { Raw = 0.0;m_avgb = MinValue; }
		...
		public void LoadPerfFrame(ref PerfFrame.PerfData cur, Course course)
		{
			...
			try
			{
				...
				if (m_Speed != cur.Speed) { m_Speed = cur.Speed; c |= StatFlags.Speed; } //  if ((validflags & StatFlags.Speed) != StatFlags.Zero) { cur.Speed = interpolate(prev.Speed, next.Speed); }
				...
				if (m_HeartRate != cur.HeartRate) { m_HeartRate = cur.HeartRate; c |= StatFlags.HeartRate; } //  if ((validflags & StatFlags.HeartRate) != StatFlags.Zero) { cur.HeartRate = interpolate(prev.HeartRate, next.HeartRate); }
		...
		public void UpdateStats( RM1.IStats istats, double splittime )
		{
			...
			try
			{
				...
				if (chgear)
				{
					...
					if (tt != m_GearInches) { m_GearInches = tt; OnPropertyChanged("GearInches"); }
				}
		...
		public String Gearing_String { get { return GearingString; } }
	}
}
